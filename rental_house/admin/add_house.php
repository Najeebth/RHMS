...
$sql = "INSERT INTO room_rental_registrations (fullname, email, mobile, alternat_mobile, plot_number, rooms, country, state, city, address, landmark, rent, sale, deposit, description, image, accommodation, vacant, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

// Prepare the statement
$stmt = mysqli_prepare($con, $sql);
if ($stmt) {
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "ssssssssssssssssssi", $fullname, $email, $mobile, $alternat_mobile, $plot_number, $rooms, $country, $state, $city, $address, $landmark, $rent, $sale, $deposit, $description, $target_file, $accommodation, $vacant, $user_id);
    // Execute the statement
    if (mysqli_stmt_execute($stmt)) {
      $image = "INSERT INTO `images`( `house_name`, `image`, `image1`, `image2`) VALUES (' $fullname','$target_file','$target_file1','$target_file2')";
      $execute = mysqli_query($con , $image);
