...
//reciever id//
        ...
        $sql="SELECT `tenant_id` FROM `tenant` WHERE `u_name` = '$selectedName'";
        $result=mysqli_query($con,$sql);
        $row = mysqli_fetch_assoc($result);
        $reciever_id=$row['tenant_id'];
...
$(document).ready(function() {
    ...
    <?php
    $sql = "SELECT u_name FROM tenant";
    $result = mysqli_query($con, $sql);

    while ($row = mysqli_fetch_assoc($result)) {
        ?>
        options += '<option value="<?php echo $row['u_name']; ?>"><?php echo $row['u_name']; ?></option>';
        <?php
    }
    ?>}
