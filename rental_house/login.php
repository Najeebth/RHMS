...
if(isset($_POST["login"])){
  ...
  if ((mysqli_num_rows($query) == 1) or (mysqli_num_rows($query1) == 1)) {
    ...
    elseif ($role == "Manager") {
      ...
      if ($stat == 0) {
        ...
        echo '<style>body{display:none;}</style>';
        echo '<script>window.location.href = "user/pay.php";</script>';
      }elseif ($stat == 1) {
        $_SESSION['username'] = $uname;
        $_SESSION['identity'] = $id;
        echo "<script type='text/javascript'>alert('Welcome $fname $lname!');</script>";
        ...
        echo '<script>window.location.href = "user/home.php.php";</script>';
